 ...
 */
export function isPrivateOrReservedIPv4Number(ipNum: number): boolean {
  ...
  if (octet1 === 192 && octet2 === 0 && octet3 === 0) return true;
  // 192.0.2.0/24 (TEST-NET-1)
  if (octet1 === 192 && octet2 === 0 && octet3 === 2) return true;
  // 192.88.99.0/24 (6to4 Relay Anycast)
  if (octet1 === 192 && octet2 === 88 && octet3 === 99) return true;
 ...
 */
export function isPrivateOrReservedIPv6(rawIpv6: string): boolean {
    ...
    if (parts.length <= 2) {
      if (parts.length === 2) {
        ...
        if (!isNaN(p1) && !isNaN(p2)) {
          ...
          if (isPrivateOrReservedIPv4Number(ipNum)) return true;
        }
      } else if (parts.length === 1) {
        const p1 = parseInt(parts[0], 16);
        if (!isNaN(p1) && isPrivateOrReservedIPv4Number(p1)) return true;
      }
 ...
 */
export async function validateSafeDownloadUrlAsync(
  urlStr: string,
  dnsLookupFn?: (host: string) => Promise<string[]>
): Promise<boolean> {
  if (!isSafeRemoteDownloadUrl(urlStr)) return false;
  ...
  try {
    ...
    if (dnsLookupFn) {
      ...
      for (const ip of ips) {
        ...
        if (parsedV4 !== null) {
          if (isPrivateOrReservedIPv4Number(parsedV4)) return false;
        } else if (ip.includes(':')) {
          if (isPrivateOrReservedIPv6(ip)) return false;
        }
      }
    }
