 ...
 */
export function buildAudioFilters(audioSettings, totalDur) {
    const af = [];
    if (!audioSettings)
        return af;
    ...
    if (audioSettings.reverbPreset && audioSettings.reverbPreset !== 'none') {
        ...
        switch (audioSettings.reverbPreset) {
            ...
            case 'grandMosque':
                ...
                break;
            case 'makkahHaram':
                af.push(`aecho=0.85:${Math.min(0.94, 0.7 + level * 0.25).toFixed(2)}:80|180|360|600:0.45|0.35|0.25|0.15`);
                break;
            case 'celestialEcho':
                af.push(`aecho=0.85:${Math.min(0.96, 0.75 + level * 0.22).toFixed(2)}:100|250|500|850:0.5|0.4|0.3|0.2`);
                break;
        }
    }
