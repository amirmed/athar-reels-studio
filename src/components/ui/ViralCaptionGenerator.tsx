    ...
    <div className="space-y-4 animate-in">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="p-1.5 rounded-lg bg-surface-800/80 hover:bg-surface-700 text-white/60 hover:text-white transition-all flex items-center gap-1 text-[11px]"
      ...
      <div className="relative rounded-2xl bg-surface-950/90 border border-white/[0.08] p-3.5 space-y-3">
        <div className="flex items-center justify-between pb-2 border-b border-white/[0.06]">
          <span className="text-xs font-bold text-gold-400 flex items-center gap-1">
            <FileText size={12} />
            معاينة نص المنشور (قابلة للتعديل)
          </span>
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-surface-800 text-white/50 font-mono">
        ...
        <textarea
          ...
          rows={7}
          className="w-full bg-transparent text-white/90 text-xs leading-relaxed focus:outline-none resize-none select-text cursor-text"
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 shadow-lg transition-all ${
              copiedType === 'all'
                ...
                : 'bg-gradient-to-r from-gold-400 to-accent-500 hover:from-gold-300 hover:to-accent-400 text-black shadow-gold-500/20 hover:scale-[1.02]'
            }`}
          >
          ...
          <button
            ...
            className={`py-2.5 px-3 rounded-xl text-xs font-bold border transition-all flex items-center gap-1 shrink-0 ${
              copiedType === 'tags'
                ...
                : 'bg-surface-800 hover:bg-surface-700 border-white/[0.08] text-white/80'
            }`}
