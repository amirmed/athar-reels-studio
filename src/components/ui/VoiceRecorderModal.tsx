...
export const VoiceRecorderModal: React.FC<VoiceRecorderModalProps> = ({
  ...
  const handleStartRecord = async () => {
    try {
      ...
      addToast({ message: t('voiceRecorder.recStarted', 'بدأ التسجيل.. اقرأ بخشوع 🎙️'), type: 'info' });
    } catch {
      addToast({ message: t('voiceRecorder.micAccessError', 'تعذر الوصول إلى المايكروفون. يرجى منح الإذن.'), type: 'error' });
    }
  };
  ...
  const handleStopRecord = async () => {
    ...
    try {
      ...
      addToast({
        message: t('voiceRecorder.recStopped', 'تم إيقاف التسجيل بنجاح! يمكنك الآن تجربة صدى المسجد ✨'),
        type: 'success',
      });
    } catch {
      addToast({ message: t('voiceRecorder.recError', 'حدث خطأ أثناء معالجة التسجيل'), type: 'error' });
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setAudioBlobUrl(result.url);
      setAudioDuration(result.duration);
      addToast({ message: t('voiceRecorder.uploadSuccess', `تم رفع ملف «${file.name}» بنجاح! 🎵`), type: 'success' });
    } catch {
      addToast({ message: t('voiceRecorder.uploadError', 'تعذر تحميل الملف الصوتي، يرجى اختيار ملف MP3 أو WAV'), type: 'error' });
    }
  };
  ...
  const handleApply = async () => {
    if (!audioBlobUrl) {
      addToast({ message: t('voiceRecorder.noAudioError', 'يرجى تسجيل صوت أو رفع ملف أولاً'), type: 'warning' });
      return;
    }
    ...
    if (!permanentUrl) {
      addToast({
        message: t('voiceRecorder.saveError', '⚠️ تعذر حفظ المقطع الصوتي بشكل دائم في مساحة التخزين. يرجى إعادة المحاولة.'),
        type: 'error',
      });
      return;
    }
    ...
    addToast({
      message: t('voiceRecorder.applySuccess', 'تم تطبيق تلاوتك الخاصة وحفظها بشكل دائم في التطبيق! 🎙️✨'),
      type: 'success',
    });
    onClose();
  };
  ...
  const reverbPresetsList: { id: MosqueReverbPreset; name: string; icon: string; desc: string }[] =
    [
      { id: 'none', name: t('voiceRecorder.reverbNone', 'بدون صدى'), icon: '🎙️', desc: t('voiceRecorder.reverbNoneDesc', 'استوديو نقي وجاف') },
      { id: 'smallRoom', name: t('voiceRecorder.reverbRoom', 'غرفة هادئة'), icon: '🏠', desc: t('voiceRecorder.reverbRoomDesc', 'صدى طبيعي ناعم (1.0s)') },
      { id: 'grandMosque', name: t('voiceRecorder.reverbMosque', 'المسجد الكبير'), icon: '🕌', desc: t('voiceRecorder.reverbMosqueDesc', 'صدى قباب المساجد (3.0s)') },
      { id: 'makkahHaram', name: t('voiceRecorder.reverbHaram', 'الحرم المكي'), icon: '🕋', desc: t('voiceRecorder.reverbHaramDesc', 'صدى الحرم الشريف (4.8s)') },
      {
        id: 'celestialEcho',
        name: t('voiceRecorder.reverbCelestial', 'صدى إيماني واسع'),
        icon: '✨',
        desc: t('voiceRecorder.reverbCelestialDesc', 'أجواء روحانية واسعة (6.5s)'),
      },
    ];
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={t('voiceRecorder.modalTitle', 'استوديو تسجيل التلاوة وصدى المسجد 🎙️🕌')}
      subtitle={t('voiceRecorder.modalSubtitle', 'سجل صوتك أو ارفع تلاوتك مع محاكاة صدى الحرم المكي وفلاتر الاستوديو الاحترافية')}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2 p-1 rounded-2xl bg-surface-900/80 border border-white/[0.06]">
          ...
          >
            <Radio size={15} />
            <span>{t('voiceRecorder.tabDirectRecord', 'تسجيل مباشر بالمايكروفون 🎙️')}</span>
          </button>
          ...
          >
            <Upload size={15} />
            <span>{t('voiceRecorder.tabUploadAudio', 'رفع ملف صوتي خارجي (MP3 / WAV) 📁')}</span>
          </button>
        </div>
          ...
          <div className="p-6 rounded-3xl bg-gradient-to-b from-surface-900/90 to-surface-950 border border-surface-700/40 text-center space-y-4 shadow-xl relative overflow-hidden">
            ...
            <div>
              ...
              <p className="text-xs text-surface-400 mt-1">
                {isRecording
                  ? t('voiceRecorder.recordingNow', 'جاري التسجيل الآن.. اضغط المرفع للإيقاف')
                  : t('voiceRecorder.clickToRecord', 'اضغط على زر المايكروفون لبدء التسجيل')}
              </p>
            </div>
            ...
            {isRecording && (
              <div className="max-w-xs mx-auto space-y-1">
                ...
                <span className="text-[11px] text-surface-400 font-mono">
                  {t('voiceRecorder.audioLevel', 'مستوى الصوت:')} {audioLevel}%
                </span>
              </div>
            )}
          </div>
          ...
          >
            ...
            <div>
              <h4 className="text-sm font-bold text-surface-50 group-hover:text-gold-300 transition-colors">
                {t('voiceRecorder.dropAudioTitle', 'اختر ملفاً صوتياً أو اسحبه هنا')}
              </h4>
              <p className="text-xs text-surface-400 mt-1">
                {t('voiceRecorder.dropAudioSubtitle', 'يدعم صيغ MP3, WAV, M4A, AAC بأعلى جودة')}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 p-4 rounded-3xl bg-surface-900/90 border border-gold-500/20 shadow-xl">
            ...
            <div className="flex items-center justify-between p-3 rounded-2xl bg-surface-950 border border-surface-700/40">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-xs font-bold text-surface-50 block">
                    {isPlaying
                      ? t('voiceRecorder.playingPreview', 'تشغيل المعاينة المباشرة مع الصدى...')
                      : t('voiceRecorder.previewEffects', 'معاينة التسجيل مع التأثيرات')}
                  </span>
                  <span className="text-xs text-gold-400 font-mono">
                    {t('voiceRecorder.durationLabel', 'المدة:')} {formatSeconds(audioDuration)}
                  </span>
                </div>
              </div>
              ...
              <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-300">
                {t('voiceRecorder.readyBadge', 'جاهز للتطبيق ✓')}
              </span>
            </div>
            ...
            <div className="space-y-2.5 pt-2 border-t border-surface-700/40">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-surface-50 flex items-center gap-1.5">
                  ...
                  <span>{t('voiceRecorder.reverbTitle', 'صدى المسجد الحرام (Mosque Spatial Reverb) 🕌')}</span>
                </label>
              ...
              {reverbPreset !== 'none' && (
                <div className="pt-2">
                  <div className="flex items-center justify-between text-xs font-bold text-surface-300 mb-1">
                    <span>{t('voiceRecorder.reverbDepth', 'قوة الصدى وعمق الارتداد')}</span>
            ...
            <div className="space-y-2 pt-2 border-t border-surface-700/40">
              <label className="text-xs font-bold text-surface-50 flex items-center gap-1.5">
                ...
                <span>{t('voiceRecorder.masteringTitle', 'فلاتر الاستوديو ونقاء الصوت (Mastering & Clarity) 🎛️')}</span>
              </label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                >
                  <div>
                    <span className="text-xs font-bold block">{t('voiceRecorder.noiseGate', '⚡ عزل الضوضاء')}</span>
                ...
                >
                  <div>
                    <span className="text-xs font-bold block">{t('voiceRecorder.clarity', '💎 نقاء التجويد')}</span>
                ...
                >
                  <div>
                    <span className="text-xs font-bold block">{t('voiceRecorder.warmth', '🎙️ دفء الصوت')}</span>
              ...
              <div className="p-3 rounded-2xl bg-surface-950/70 border border-amber-500/30 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-surface-50 flex items-center gap-1">
                    ...
                    <span>{t('voiceRecorder.pitchPolishTitle', 'تنعيم النبرة والهارمونيك (Auto-Pitch Polish) 💎')}</span>
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    {enablePitchPolish ? t('voiceRecorder.enabled', 'مفعل ✓') : t('voiceRecorder.disabled', 'معطل')}
                  </button>
                </div>
                {enablePitchPolish && (
                  <input
      ...
      <div className="pt-3 border-t border-surface-700/40 flex items-center justify-between shrink-0">
        ...
        >
          {t('voiceRecorder.cancelBtn', 'إلغاء')}
        </button>
        ...
        {audioBlobUrl && (
          ...
          >
            <Zap size={15} />
            <span>{t('voiceRecorder.applyBtn', 'استخدام هذا التسجيل في الريلز 🚀')}</span>
          </button>
        )}
      </div>
    </Modal>
