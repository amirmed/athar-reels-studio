...
ipcMain.handle('projects:save', async (_event, projectOrProjects) => {
    try {
        const sanitizeForDisk = (item) => {
...
ipcMain.handle('projects:saveAll', async (_event, projects) => {
    try {
        const sanitizeForDisk = (item) => {
            if (!item || typeof item !== 'object')
                return item;
...
ipcMain.handle('exports:save', async (_event, jobs) => {
    try {
        const exportsFile = path.join(getAppDataPath(), 'exports.json');
...
ipcMain.handle('fs:writeBinaryFile', async (_event, filePath, data) => {
    try {
        if (!isSafeUserPath(filePath)) {
            return { success: false, error: 'Access denied: Path outside allowed directories' };
        }
...
// ==================== Shell & App ====================
ipcMain.handle('audio:getTTSStream', async (_event, text, voice = 'ar-SA-HamedNeural') => {
    try {
