...
import { buildAudioFilters } from '../src/services/audioDspFilters';
...
function getMediaExtension(source) {
    try {
...
function inferBackgroundKind(source, contentType) {
    if (!source && !contentType)
        return undefined;
    return mediaKindFromContentType(contentType) || (source ? mediaKindFromExtension(getMediaExtension(source)) : undefined);
}
function mediaKindFromFile(filePath) {
    let fd;
    try {
        fd = fs.openSync(filePath, 'r');
        const buffer = Buffer.alloc(16);
        const bytes = fs.readSync(fd, buffer, 0, buffer.length, 0);
        if (bytes >= 12 && buffer.toString('ascii', 4, 8) === 'ftyp')
            return 'video';
        if (bytes >= 4 && buffer[0] === 0x1a && buffer[1] === 0x45 && buffer[2] === 0xdf && buffer[3] === 0xa3)
            return 'video';
        if (bytes >= 3 && buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff)
            return 'image';
        if (bytes >= 8 && buffer[0] === 0x89 && buffer.toString('ascii', 1, 4) === 'PNG')
            return 'image';
        if (bytes >= 12 && buffer.toString('ascii', 0, 4) === 'RIFF' && buffer.toString('ascii', 8, 12) === 'WEBP')
            return 'image';
        if (bytes >= 2 && buffer.toString('ascii', 0, 2) === 'BM')
            return 'image';
        if (bytes >= 6 && buffer.toString('ascii', 0, 3) === 'GIF')
            return 'video';
    }
    catch {
        return undefined;
        ...
        if (fd !== undefined) {
            try {
                ...
                console.debug('[MediaKind] close error:', e);
            }
        }
    }
    return undefined;
}
function extensionForBackground(source, kind) {
    const ext = getMediaExtension(source);
    if (mediaKindFromExtension(ext))
        return ext;
...
function findArabicFont(fontFamily, weight) {
    ...
    if (bundledDir) {
        if (family.includes('cairo')) {
            candidates.push(bold ? path.join(bundledDir, 'Cairo-Bold.ttf') : path.join(bundledDir, 'Cairo-Regular.ttf'), path.join(bundledDir, 'Cairo-Regular.ttf'));
        }
        else if (family.includes('tajawal')) {
            candidates.push(bold ? path.join(bundledDir, 'Tajawal-Bold.ttf') : path.join(bundledDir, 'Tajawal-Regular.ttf'), path.join(bundledDir, 'Tajawal-Regular.ttf'));
        }
        else if (family.includes('scheherazade')) {
            candidates.push(bold ? path.join(bundledDir, 'ScheherazadeNew-Bold.ttf') : path.join(bundledDir, 'ScheherazadeNew-Regular.ttf'), path.join(bundledDir, 'ScheherazadeNew-Regular.ttf'));
        }
        else if (family.includes('naskh') || family.includes('noto')) {
            candidates.push(bold ? path.join(bundledDir, 'NotoNaskhArabic-Bold.ttf') : path.join(bundledDir, 'NotoNaskhArabic-Regular.ttf'), path.join(bundledDir, 'NotoNaskhArabic-Regular.ttf'));
        }
        else {
            // Default to Amiri (Authentic Naskh Quranic font)
            candidates.push(bold ? path.join(bundledDir, 'Amiri-Bold.ttf') : path.join(bundledDir, 'Amiri-Regular.ttf'), path.join(bundledDir, 'Amiri-Regular.ttf'), path.join(bundledDir, 'ScheherazadeNew-Regular.ttf'), path.join(bundledDir, 'Cairo-Regular.ttf'));
        }
    }
    ...
    if (family.includes('amiri') || family.includes('scheherazade') || family.includes('naskh') || family.includes('lateef')) {
        candidates.push(bold ? 'C:/Windows/Fonts/tradbdo.ttf' : 'C:/Windows/Fonts/trado.ttf', 'C:/Windows/Fonts/arabtype.ttf');
    }
        ...
        candidates.push(bold ? 'C:/Windows/Fonts/tahomabd.ttf' : 'C:/Windows/Fonts/tahoma.ttf', bold ? 'C:/Windows/Fonts/arialbd.ttf' : 'C:/Windows/Fonts/arial.ttf');
    }
    candidates.push(bold ? 'C:/Windows/Fonts/arialbd.ttf' : 'C:/Windows/Fonts/arial.ttf', 'C:/Windows/Fonts/tahoma.ttf', 'C:/Windows/Fonts/trado.ttf', 'C:/Windows/Fonts/arabtype.ttf');
...
function hexToAssColor(hex, fallback, alphaHex = '00') {
...
function generateAssSubtitleFile(ayahs, options, w, h, assFilePath) {
    ...
    ayahs.forEach((ayah) => {
        ...
        if (options.surahName) {
            ...
            lines.push(`Dialogue: 1,${formatAssTime(ayahStart)},${formatAssTime(ayahEnd)},SurahBadge,,0,0,0,,${surahTitle}`);
        }
        // 2. Watermark at bottom
        if (options.watermark) {
            lines.push(`Dialogue: 1,${formatAssTime(ayahStart)},${formatAssTime(ayahEnd)},Watermark,,0,0,0,,${options.watermark}`);
        }
        // 3. Translation at bottom if enabled
        if (options.showTranslation && ayah.translationText) {
            lines.push(`Dialogue: 1,${formatAssTime(ayahStart)},${formatAssTime(ayahEnd)},Translation,,0,0,0,,${ayah.translationText.replace(/\r?\n/g, ' ')}`);
        }
        ...
        if (enableHighlight && rawWords.length > 0) {
            ...
            wordChunks.forEach((chunk, cIdx) => {
                ...
                for (let wIdx = 0; wIdx < chunk.length; wIdx++) {
                    ...
                    const ayahNumSuffix = (ayah.numberInSurah && isLastChunk && isLastWordInChunk)
                        ...
                        : '';
                    lines.push(`Dialogue: 2,${formatAssTime(wStart)},${formatAssTime(wEnd)},QuranDefault,,0,0,0,,${formattedChunkText}${ayahNumSuffix}`);
                }
            });
        }
        else {
            ...
            lines.push(`Dialogue: 2,${formatAssTime(ayahStart)},${formatAssTime(ayahEnd)},QuranDefault,,0,0,0,,${textToDisplay}`);
        }
    });
    try {
...
function wrapTextByChars(text, maxChars, maxLines = 4) {
...
function drawTextFilter(text, x, y, fontSize, fontColor, fontPath, enable, extra = '') {
...
function getContentCenterY(settings, h) {
    const position = settings?.position || 'center';
    if (position === 'top')
        return Math.round(h * 0.30);
    if (position === 'bottom')
        return Math.round(h * 0.70);
    return Math.round(h * 0.50);
}
function getTextXExpression(settings, w) {
    const align = settings?.textAlign || 'center';
    if (align === 'right')
        return `w-text_w-${Math.round(w * 0.12)}`;
    if (align === 'left')
        return `${Math.round(w * 0.12)}`;
    return '(w-text_w)/2';
}
function buildTextAlpha(start, end, transition) {
...
function addVideoEffectFilters(filters, effect, w, h) {
...
function addPreviewOverlayFilters(filters, ayahs, options, w, h) {
    const settings = options.textSettings || {};
    const previewWidth = previewWidths[options.aspectRatio] || 270;
...
function safeSendProgress(sender, data) {
    try {
...
export function setupExportHandlers(tempDir) {
    let activeFfmpegCmd = null;
    let isNativeExportActive = false;
    let activeJobTempDir = null;
    ipcMain.handle('export:choosePath', async (_event, projectName) => {
    ...
    ipcMain.handle('export:start', async (_event, options) => {
        ...
        try {
            ...
            const isLocalFile = (u) => typeof u === 'string' &&
                !u.startsWith('blob:') &&
                !u.startsWith('http://') &&
                !u.startsWith('https://') &&
                !u.startsWith('data:') &&
            ...
            let syncedTotalDuration = options.totalDuration ||
